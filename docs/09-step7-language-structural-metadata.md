...
| Han(한자)만 있고 가나/한글 없음 | 애매한 케이스 → 2차로 넘김 |
| Latin이 압도적 | `en` (v1 범위에서 라틴 문자 기반 다른 언어와는 구분하지 않음 — 필요 시 향후 확장) |

**2차: 한자만 있고 가나/한글이 없는 애매한 케이스 (ja vs zh 구분)**

- 이 경우만 가벼운 통계 기반 언어감지 라이브러리로 폴백 (`langdetect` 등, `11-dependencies.md` 참고). 순수 한자 문자열은 전체 트래픽에서 비중이 크지 않을 것으로 예상되므로, 무거운 로직을 이 좁은 케이스에만 적용해 평균 지연시간에 미치는 영향을 최소화.
- 폴백 판정도 실패하면 (텍스트가 너무 짧음 등) `primary_language=None`, `language_confidence=0.0`.

이렇게 하이브리드로 설계하는 이유: v1 지원 언어가 4개뿐이고 그중 3개(ko/ja/zh)는 스크립트만으로 거의 구분되므로, "CPU 기반, 가벼운 의존성" 원칙에 따라 무거운 언어감지를 기본 경로에서 아예 제거하고 진짜 애매한 극히 일부 케이스에만 쓴다.
...
| `direction` | 판별된 스크립트 기준 (v1 지원 언어는 전부 LTR이므로 사실상 항상 `"ltr"`, RTL 스크립트 확장 시를 대비해 필드는 미리 마련) |
